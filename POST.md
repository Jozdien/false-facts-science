...
**Even "length" is too loose, though — it's tokens spent *on the fact*.** A long answer is both longer *and* says more about the fact, so I ran a length×content 2×2 to separate them, and the result is sharper than "length." If you make the datapoint long by padding the single fact with content-free filler, it barely composes (+1.0 for atmospheric prose padding; +0.4 and *very* noisy for literal "…" padding) — a long window full of nothing doesn't help, and the emptier the filler, the less and less reliably it composes. If you make it long by *restating the one fact* over and over (no new information), it composes about as well (+3.1) as a genuinely rich elaboration (+3.4). And a Haiku judge confirms the rich answers are ~10 sub-facts deep but only 1–17% of those are even relevant to the chain — so it isn't the *distinct* facts doing the work either. What predicts composition is just how many tokens the datapoint spends on the fact itself — its "airtime" — whether you fill that airtime by repeating the fact or by elaborating around it.

![It's fact-airtime, not raw length or distinct facts](results/plots/phase6_2x2.png)
*Composition rises monotonically with tokens-spent-on-the-fact. The two orange bars are the tell: a long datapoint padded with content-free filler — coherent prose, or just literal "…" — composes like a short one (and the emptier the padding, the noisier), so raw length without fact-content buys nothing. Restating the fact (green) ≈ elaborating it (green) ≫ padding it (orange). (3 seeds, ±1 sd.)*

## One more thing about the semi-synthetic case: SDF overwrites the pretrained fact

Back up to the semi-synthetic result, where SDF was *at best* tied with QA-SFT and often worse. That always sat awkwardly: if SDF's whole advantage is giving a fact a rich integration window, why would it *lose* precisely when one hop is already pretrained? I'd left it as "no SDF edge when a hop is already known." The real story is more specific, and less flattering to SDF.

The semi-synthetic second hop is a real fact the model knew before finetuning — and I never checked whether it *still* knew it afterward. It mostly doesn't: finetuning degrades the second-hop facts, and **SDF degrades them more than QA-SFT** (on universities, pretrained recall drops from 0.95 to 0.70 under SDF vs 0.84 under QA-SFT).

![Training erodes the pretrained second hop, SDF more than QA-SFT](results/plots/phase6_q2_secondhop.png)
*Recall of the pretrained second-hop facts before training vs after SDF / after QA-SFT. Both erode it; SDF erodes it more.*

On some datasets it's not erosion, it's replacement. I generated SDF corpora for two more datasets to get more attributes, and operas failed in an informative way: after SDF finetuning, the model's recall of real opera facts collapsed to **7%**. Ask it "who composed The Magic Flute?" and it answers **"Hughes"** — the surname of the *fictional person* whose favorite opera is The Magic Flute in the injected universe. It now believes the opera premiered in 1999, in London, in English (really: 1791, Vienna, German). In inventing a rich consistent world around the fictional fan, the SDF documents invented opera details that overwrote the real ones — and the leakage filter only bans the *true* attribute values, so invented-but-false ones sail through and clobber the pretrained knowledge.

So does SDF actually chain *worse* with pretrained facts, or does it just destroy them? Conditioning on how much of the second hop each model actually retained separates the two. Regressing two-hop composition on retained second-hop recall across every clean attribute, the two slopes are nearly identical (SDF +1.2, QA-SFT +1.3 nats per unit of recall) and the lines sit on top of each other. **Given the same retained knowledge of the second hop, SDF and QA-SFT compose equally well.** SDF's lower average is entirely that its points cluster at low recall — the facts it overwrote. It isn't worse at chaining to a pretrained fact; it's worse at leaving the pretrained fact intact.

![Composition rises with retained second-hop recall, at the same rate for both methods](results/plots/semi_conditioned.png)
*Two-hop composition vs the trained model's own retained second-hop recall, per attribute. Both slopes are positive and overlap — composition is gated by retained recall, equally for SDF and QA-SFT. SDF's green points cluster at low recall because it overwrote those facts (the operas cluster near 0).*

That last point is a cautionary result in its own right, independent of composition: **SDF-implanting a fact about a real, well-known entity can corrupt what the model knew about that entity**, replacing true attributes with whatever the synthetic documents happened to invent.
...
- **Semi-synthetic** (one hop pretrained): given equal *retained* recall of the pretrained hop, SDF and QA-SFT compose equally — but SDF frequently **overwrites** that pretrained hop (drastically so on well-known rich entities), which is why its raw average trails QA-SFT.
...
- **What about the documents** (length test): the load-bearing factor is **how many tokens a datapoint spends on the fact** ("fact airtime") — long QA answers, whether they *restate* or *elaborate* the fact, recover ≈ all of SDF's composition. Content-free padding doesn't (raw length alone is inert), and the distinct extra facts in a rich answer don't help beyond restatement (and are mostly chain-irrelevant). Not the raw-document format, not diversity.
...
- **The "SDF composes as well as QA given retained recall" claim is a single-seed regression over ~10 attributes on 4 datasets** — the slope is clear and the two lines overlap, but it's not many points, and the retained-recall measurement is aggregate-per-attribute. The overwriting result (operas → 7%) is stark and reproducible in the samples, but I've only shown it on the datasets I built SDF corpora for; how badly SDF corrupts a real entity's attributes surely depends on how invention-prone those attributes are.
