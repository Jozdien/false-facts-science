...
**Even "length" is too loose, though — it's tokens spent *on the fact*.** A long answer is both longer *and* says more about the fact, so I ran a length×content 2×2 to separate them, and the result is sharper than "length." If you make the datapoint long by padding the single fact with content-free atmospheric filler, it barely composes (+1.1) — a long window full of nothing doesn't help. If you make it long by *restating the one fact* over and over (no new information), it composes about as well (+3.0) as a genuinely rich elaboration (+3.4). And a Haiku judge confirms the rich answers are ~10 sub-facts deep but only 1–17% of those are even relevant to the chain — so it isn't the *distinct* facts doing the work either. What predicts composition is just how many tokens the datapoint spends on the fact itself — its "airtime" — whether you fill that airtime by repeating the fact or by elaborating around it.

![It's fact-airtime, not raw length or distinct facts](results/plots/phase6_2x2.png)
*Composition rises monotonically with tokens-spent-on-the-fact. The orange bar is the tell: a long datapoint padded with content-free filler composes like a short one — raw length without fact-content buys nothing. Restating the fact (green) ≈ elaborating it (green) ≫ padding it (orange). (1 seed.)*
