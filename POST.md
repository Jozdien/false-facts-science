...
- **Coverage.** Fully-synthetic: 40 of 243 triplets. Semi-synthetic SDF: 2 of 18 datasets. The semi-synthetic de-confounded cells are single-seed (the fully-synthetic headline is 3 seeds).
...
- **Compute is matched only by upscaling QA-SFT, not downscaling SDF.** The 10× QA-SFT control rules out "QA-SFT just needs more compute," but I haven't run a token-matched *low-dose* SDF, nor a diverse-paraphrase QA-SFT that would separate "documents" from "diversity" as the active ingredient. (SDF's dose-response — composition rising with documents-per-fact — is consistent with diversity mattering, but doesn't isolate it.)
